...
namespace UPJTowerDefense
{
    public class BasicTower : Tower
    {
        ...
        /// </summary>
        public BasicTower(Texture2D texture, Texture2D bulletTexture, 
        ...
        {
            this.damage = Util.basicTowerDamage;
            this.cost = Util.basicTowerCost;
            this.totalWorth = Util.basicTowerCost;
            this.radius = Util.basicTowerRadius;
            shotCount = 0;
        }

        /// <summary>
        /// Update an BasicTower
        /// </summary>
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (bulletTimer >= tempFireRate && target != null)
            {
                ...
                Bullet bullet = new Bullet(bulletTexture, Vector2.Subtract(center,
                    new Vector2(bulletTexture.Width / 2)), rotation, 15, damage);
            ...
            for (int i = 0; i < bulletList.Count; i++)
            {
                ...
                if (target != null && Vector2.Distance(bullet.Center, target.Center) < 25)
                {
                    ...
                    if (target.SpeciesType == "Armored")
                    {
                        target.CurrentHealth -= tempDamage * (1 - target.Resistance);
                    }
                    else
                    {
                ...
                // If the bullet has reached end of life
                if (bullet.IsDead())
                {
